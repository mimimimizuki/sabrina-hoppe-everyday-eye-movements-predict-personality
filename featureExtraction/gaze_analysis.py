...
class gazeAnalysis (object):
	...
	def get_ngram_features(self, wb):
		feat = []
		feat.append(sum(x > 0 for x in list(wb.values())))  # 1. size
		feat.append(np.max(list(wb.values())))  # 2. maximum
		nonzeros = [i for i in list(wb.values()) if i]
		if len(nonzeros)<1:
			feat.append(0)
		else:
			feat.append(min(nonzeros))  # 3. non-zero minimum
		feat.append(np.argmax(list(wb.values())))  # 2. arg max
		if len(nonzeros)<1:
			feat.append(0)
		else:
			feat.append(np.argmin(np.array(nonzeros)))  # 3. arg min
		feat.append(feat[1] - feat[2])  # 4. diff max - min
		feat.append(np.mean(list(wb.values())))  # 5. mean of all counts
		feat.append(np.var(list(wb.values())))  # 6. var of all counts
		return feat
